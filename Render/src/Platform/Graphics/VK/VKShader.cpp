...
void SampleRender::VKShader::AllocateTexture(TextureElement textureElement)
{
	...
	vkr = vkCreateImage(device, &imageInfo, nullptr, &m_Textures[textureElement.GetBindingSlot()].Resource);
	assert(vkr == VK_SUCCESS);

	VkMemoryRequirements memRequirements;
	vkGetImageMemoryRequirements(device, m_Textures[textureElement.GetBindingSlot()].Resource, &memRequirements);
	...
	vkr = vkAllocateMemory(device, &allocInfo, nullptr, &m_Textures[textureElement.GetBindingSlot()].Memory);
	assert(vkr == VK_SUCCESS);

	vkr = vkBindImageMemory(device, m_Textures[textureElement.GetBindingSlot()].Resource, m_Textures[textureElement.GetBindingSlot()].Memory, 0);
	assert(vkr == VK_SUCCESS);

	VkImageViewCreateInfo viewInfo{};
	viewInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
	viewInfo.image = m_Textures[textureElement.GetBindingSlot()].Resource;
	...
	vkr = vkCreateImageView(device, &viewInfo, nullptr, &imageView);
	assert(vkr == VK_SUCCESS);
}
...
void SampleRender::VKShader::CopyTextureBuffer(TextureElement textureElement)
{
	...
	barrier.image = m_Textures[textureElement.GetBindingSlot()].Resource;
	...
	vkCmdCopyBufferToImage(m_CopyCommandBuffer, stagingBuffer, m_Textures[textureElement.GetBindingSlot()].Resource, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
...
void SampleRender::VKShader::CreateDescriptorSet(TextureElement textureElement)
{
	VkResult vkr;
	auto device = (*m_Context)->GetDevice();
	...
	vkr = vkAllocateDescriptorSets(device, &allocInfo, &m_TexturesTables[textureElement.GetBindingSlot()].Descriptor);
	assert(vkr == VK_SUCCESS);

	VkDescriptorImageInfo imageInfo{};
	imageInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
	imageInfo.imageView = m_Textures[textureElement.GetBindingSlot()].View;
	imageInfo.sampler = m_Samplers[3];

	VkWriteDescriptorSet descriptorWrite{};
	descriptorWrite.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
	descriptorWrite.dstSet = m_TexturesTables[textureElement.GetBindingSlot()].Descriptor;
